...
const AttendanceUpload = () => {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            setFile(null); // Clear file after successful (or partially successful) upload

            // Optional: trigger a refresh of dashboard data here if needed,
            // e.g., by firing an event or calling a context function.
    ...
    const downloadTemplate = () => {
        ...
        // Create sample data matching the exact required format
        const wsData = [
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 flex flex-col gap-4">
            <div className="flex items-center justify-between">
                ...
                <button
                    ...
                    className="text-sm flex items-center gap-2 text-indigo-600 hover:text-indigo-800 font-medium transition-colors"
                >
            ...
            <div className="mt-4 border-2 border-dashed border-slate-200 rounded-lg p-6 flex flex-col items-center justify-center bg-slate-50 hover:bg-slate-100 transition-colors">
                ...
                <label className="cursor-pointer">
                    <span className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-indigo-700 transition-colors shadow-sm">
                        Select Excel File
                    </span>
                    <input
                        type="file"
                ...
                <div className={`mt-4 p-4 rounded-lg flex flex-col gap-3 ${success ? 'bg-emerald-50 border border-emerald-100' : 'bg-red-50 border border-red-100'}`}>
                    <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                            <p className={`text-sm font-medium ${success ? 'text-emerald-800' : 'text-red-800'}`}>
                                {message}
                            </p>

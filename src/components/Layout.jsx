...
import { LogOut, ChevronDown, Calendar as CalendarIcon, RotateCcw, Activity, UploadCloud, LayoutDashboard } from 'lucide-react';
        ...
        <div className="min-h-screen bg-slate-50 text-slate-800 font-sans flex flex-col relative">
            <header className="sticky top-0 h-20 bg-slate-900 shadow-xl z-50 px-4 md:px-8 flex items-center justify-between relative shrink-0">
                ...
                {!isUploadPage ? (
                    ...
                    <div className="flex-1 hidden xl:flex items-center justify-center z-10 px-2">
                        <span className="text-slate-400 text-sm font-semibold tracking-wider uppercase">Master Data Management</span>
                    </div>
                )}
                <div className="relative z-10 shrink-0" ref={userMenuRef}>
                    ...
                    {showUserMenu && (
                        <div className="absolute right-0 top-full mt-2 w-48 bg-slate-800 rounded-lg shadow-2xl border border-slate-700 py-2 z-[9999]">
                            <button onClick={() => { navigate('/dashboard'); setShowUserMenu(false); }} className={`w-full px-4 py-2.5 text-left text-slate-200 hover:bg-slate-700 transition-colors flex items-center space-x-3 group ${!isUploadPage ? 'bg-slate-700/30' : ''}`}>
                                <LayoutDashboard className="h-4 w-4 text-slate-400 group-hover:text-blue-400 transition-colors" />
                                <span className="font-medium group-hover:text-blue-400 transition-colors">Dashboard</span>
                            </button>
                            <button onClick={() => { navigate('/upload'); setShowUserMenu(false); }} className={`w-full px-4 py-2.5 text-left text-slate-200 hover:bg-slate-700 transition-colors flex items-center space-x-3 group border-b border-slate-700 ${isUploadPage ? 'bg-slate-700/30' : ''}`}>
                                <UploadCloud className="h-4 w-4 text-slate-400 group-hover:text-blue-400 transition-colors" />
                                <span className="font-medium group-hover:text-blue-400 transition-colors">Upload Data</span>
                            </button>
